...
out_file=Cglabrata_MEC_CGD_amr_genes
...
gatk VariantsToTable \
    -V "${out_file}".vcf.gz \
    -F CHROM -F POS -F GENE -F LOCUS -F REF -F ALT -GF GT -GF VAF \
    -O "${out_file}".table
...
# keep only the first
awk 'BEGIN{FS="|"; OFS="\t"} {print $2, $3, $10, $11}' \
    "${out_file}".ann.table > "${out_file}".first_ann.table

# fix the header
sed -i '1i Annotation\tImpact\tCoding_Change\tAA_change' \
